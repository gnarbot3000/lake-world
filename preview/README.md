...
Home is a hive feed, not a giant easy/hard list. Working range is inferred from what you landed lately (seed difficulties 1–10). Show **3 landed near your band** and **3 to try** at that same band. Search to log something not in those six (catalog match lands it; otherwise a write-in at your current band). The Kneeboard search / write-in form is full-width (input stacked above **Log it**). Cold start uses band 3 (surface tricks). Check a trick to land it. Guest Preview stays personal-only. Hive-adjusted club difficulty can come later.
...
Roster / admins live in lake.world Supabase. **Club page logs** (submitted slalom sets and kneeboard landings) are shared for approved members, with High fives and comments. Personal Mini history can stay on this device. Guest Preview is personal-only. Do not seed fake club performances.

## Recent logs vs local boards

- **Club page recent logs** — hosted, approved members only. The selected club is shown in separate Slalom and Kneeboard sections on the Club page (`preview/admin.html`), newest first. Slalom rows show name, date/time, pass (buoys · line · speed), and **Chart**. Kneeboard rows show name, date, and trick — no easy unlock / hard log mode text on this board. A comment icon, a high-five hand, and (for Kneeboard only) a filled photo icon when a club-shared photo exists sit right-aligned on that same trick (or pass · Chart) line — comment, high-five, then photo furthest right. High five is one per member per log, toggles off, and shows its count right of the hand only at 1+. The comment icon carries a small count when the log has comments, and tapping it drops the comments and the composer open under that row (tap again to collapse); comments stay Instagram-style: **Name** then body on one line, with a tiny Delete after your own, and the composer is Add a comment… plus a text Post. Tap the Kneeboard photo icon to open the club-shared viewer. Slalom rows never get a photo icon. Guests, pending, and denied users do not see these sections. Admin management appears below activity only for admins. The personal Mini has no mixed Recent logs board and no chart-preview.
...
- Legacy clip media (if any) may still live in IndexedDB (`nrs-ski-lake-tricks-v2`), keyed per skier, on this device only.
- **Kneeboard trick photos** are club-shared: private Supabase Storage bucket `kneeboard-photos` plus `kneeboard_log_photos` metadata on lake.world (`zejficslgaqryxrndfqi` only). Approved members of the selected club can view short signed URLs. Guests/pending/offline without a hosted log get no local fallback (toast **Join a club to share photos**). No photo blobs/base64/blob URLs in localStorage.
- Clearing site data for the file will wipe the roster, logbook, score, trophies, and slalom sets on this device; club-shared photos remain in Supabase for the club.
