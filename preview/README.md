...
- **Recent logs** — hosted, approved members only. Newest first, mixing submitted slalom sets and kneeboard landings. Slalom rows show name, date/time, pass (buoys · line · speed), and **Chart**. Kneeboard rows show name, date, and trick — no easy unlock / hard log mode text on this board. A comment icon and a high-five hand sit right-aligned on that same trick (or pass · Chart) line, hand furthest right. High five is one per member per log, toggles off, and shows its count right of the hand only at 1+. The comment icon carries a small count when the log has comments, and tapping it drops the comments and the composer open under that row (tap again to collapse); comments stay Instagram-style: **Name** then body on one line, with a tiny Delete after your own, and the composer is Add a comment… plus a text Post. Kneeboard rows also show a filled photo icon when a club-shared photo exists for that hosted log (tap to open the viewer). Slalom rows never get a photo icon. Guests and pending users do not see this board.
