...
The hosted Mini does **not** seed the old 30-name list. `app/club-roster.js` is unused and empty. `SEED_GEN` is `invite-only-1` (name only; leftover `seed: true` people are stripped and not replaced).
...
Seeded trophies (titles match `TROPHY_DEFS` in `app/app.js`):
